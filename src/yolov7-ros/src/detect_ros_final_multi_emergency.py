...
from visualizer import draw_detections_emergency
...
class Yolov7Publisher:
    def __init__(self, image1: str, weights: str, conf_thresh: float = 0.5,
        ...
        vis_topic = pub_topic + "visualization_emergency" if pub_topic.endswith("/") else \
            pub_topic + "/visualization_emergency"
    ...
    def process_img_msg(self, img_msg: Image):
        ...
        if self.visualization_publisher:
            ...
            ###################################
            ### publish
            # EmergencyLight on => True / off => False
            if len(classes) > 0 :
                # print(classes)
                if(1 in classes or 0 in classes or 2 in classes):
            ...
            vis_img = draw_detections_emergency(np_img_orig, bboxes, classes,
                                      self.class_labels)
            # cv2.imshow(vis_img)
