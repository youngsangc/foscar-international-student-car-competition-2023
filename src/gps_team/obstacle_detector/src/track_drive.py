...
def colorcone_callback(msg):
	...
	#print("BEFORE LENGTH : {}".format(len(sorted_colorcone)))
	sorted_colorcone = [cone for cone in sorted_colorcone if math.sqrt((cone.dist_x)**2 + (cone.dist_y)**2) < 600]
	#print("AFTER LENGTH : {}".format(len(sorted_colorcone)))	
	#print("info:",sorted_colorcone)
...
class point:
	...
	def avoid_collision(self,min_list):
		...
		margin=cal_distance(point.x,point.y)*(-10)+16
		if point.y>0:
			steer-=margin
			drive(steer,True)
		else:
	...
	def circle(self):
		...
		if len(self.obData.circles) == 0:
			...
			print("CONECONECONE",len(yellow_cone),len(blue_cone))
			...
			sorted_list_3=[1,2,3]
 			...
 			if(len(circles)==2):
				...
				if  	len(blue_cone)==0 :
					self.car_angle_deg+=5
					print("3333333333333333333YYYYYYYYYYYYYYYYYYYYYYY")
					drive(self.car_angle_deg,True)
					return 
				elif 	len(yellow_cone)==0 :
					self.car_angle_deg-=5
					print("3333333333333333333BBBBBBBBBBBBBBBBBBBBBB")
					drive(self.car_angle_deg,True)
					return
				else:	
				...
				if len(blue_cone)>= 4 and len(yellow_cone)<=1: 
				...
				elif 	len(yellow_cone)>= 5 and len(blue_cone)<=1 :					
