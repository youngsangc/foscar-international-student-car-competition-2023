  ...
  for( int iy=0 ; iy<real_line_idx ; iy++ )
	{
		...
		cv::circle(imgResult, cv::Point(real_lines_x[iy]*interval, real_lines_y[iy]*interval), 3, cv::Scalar(0, 0, 255), cv::FILLED);
    ...
    //   dy2 = data;
    // }
		cv::circle(imgResult, cv::Point(real_lines_x[iy]*interval, data*interval) ,5, cv::Scalar(0,255,0), cv::FILLED) ;
	}
