...
def Radius(rx, ry):
    ...
    if (rx[-1] - rx[3] != 0): 
        ...
        w = math.sqrt(pow((ry[-1] - ry[3]), 2) + pow((rx[-1] - rx[3]), 2))
...
def start():
    ...
    while not rospy.is_shutdown():
        ...
        angle = steering_Angle(R) *15 # 앞서 구현한 곡률반경 함수와 steering Angle값 계산함수를 이용하여 R과 angle변수를 초기화합니다. default로 좌측차선을 인식하여 주행합니다. 
        ...
        if (abs(angle)<=5 and lx[3]<180): #차량이 너무 우측차선에 쏠려있는 경우 차량의 중앙으로 정렬할 수 있게끔 방어코드를 작성하였습니다.
