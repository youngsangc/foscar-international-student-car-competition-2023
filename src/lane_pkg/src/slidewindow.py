...
class SlideWindow:
    ...
    def slidewindow(self, img, is_detected):
        ...
        pts_catch = np.array([[0, 320], [width, 320]], np.int32) ### 조향값 주는 선
        ...
        global lenofleft
        global lenofright
        lenofright = len(good_right_inds)
        lenofleft = len(good_left_inds)
        ...
        if line_flag != 3:
            ...
            for window in range(0, nwindows):
                if line_flag == 1: 
                    ...
                    cv2.rectangle(out_img, (win_x_low + int(width * term), win_y_low), (win_x_high + int(width * term), win_y_high), (255, 0, 0), 1)
                    ...
                    # 0.165 is the half of the road(0.33)
                        x_location = x_current + int(width * term/2) 
                else: # change line from left to right above(if)
                    win_y_low = y_current - (window + 1) * window_height
                    win_y_high = y_current - (window) * window_height
                    win_x_low = x_current - margin
                    win_x_high = x_current + margin
                    cv2.rectangle(out_img, (win_x_low - int(width * term), win_y_low), (win_x_high - int(width * term), win_y_high), (0, 255, 0), 1)
