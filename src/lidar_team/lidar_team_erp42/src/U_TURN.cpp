...
void dynamicParamCallback(lidar_team_erp42::uturn_hyper_parameterConfig &config, int32_t level) {
  xMinROI = config.uturn_xMinROI;
  xMaxROI = config.uturn_xMaxROI;
  yMinROI = config.uturn_yMinROI;
  yMaxROI = config.uturn_yMaxROI;
  zMinROI = config.uturn_zMinROI;
  zMaxROI = config.uturn_zMaxROI;

  minPoints = config.uturn_minPoints;
  epsilon = config.uturn_epsilon;
  minClusterSize = config.uturn_minClusterSize;
  maxClusterSize = config.uturn_maxClusterSize;

  xMinBoundingBox = config.uturn_xMinBoundingBox;
  xMaxBoundingBox = config.uturn_xMaxBoundingBox;
  yMinBoundingBox = config.uturn_yMinBoundingBox;
  yMaxBoundingBox = config.uturn_yMaxBoundingBox;
  zMinBoundingBox = config.uturn_zMinBoundingBox;
  zMaxBoundingBox = config.uturn_zMaxBoundingBox;
}
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "uturn_obstacle_detector");
  ros::NodeHandle nh;

  dynamic_reconfigure::Server<lidar_team_erp42::uturn_hyper_parameterConfig> server;
  dynamic_reconfigure::Server<lidar_team_erp42::uturn_hyper_parameterConfig>::CallbackType f;
  ...
  staticObsClusterPub = nh.advertise<sensor_msgs::PointCloud2>("/uturn_cluster", 0.001);                  
  staticObsMarkerPub = nh.advertise<visualization_msgs::MarkerArray>("/uturn_marker", 0.001);  
  staticObsPosePub = nh.advertise<lidar_team_erp42::Boundingbox>("/uturn_position", 0.001);    
  staticObsCropboxPub = nh.advertise<sensor_msgs::PointCloud2>("/uturn_cropbox", 0.001); 
  staticObsShortFlagPub = nh.advertise<std_msgs::Bool>("/uturn_flag_short", 0.001); 
  staticObsLongFlagPub = nh.advertise<std_msgs::Bool>("/uturn_flag_long", 0.001); 
